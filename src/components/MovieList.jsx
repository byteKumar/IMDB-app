...
const MovieList = () => {
  ...
  const loadMovieByPage = (pageNo) => {
    fetch(
      `https://api.themoviedb.org/3/movie/popular?api_key=0b5415eb9bf023d556ef265b425e0e4a&language=en-US&page=${pageNo}`
    )
      .then((res) => res.json())
        ...
        {movies.map((movie) => (
          <div className="movie">
            ...
            {favourites[movie.id] ? (
              ...
              <button onClick={() => onAdd(movie)}>Add to Favourite</button>
            )}
          </div>
        ))}
      </div>
